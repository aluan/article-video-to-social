...
3. **发布到社交平台**
   ...
   **平台格式优化**（发布前必做）：
   - **微信公众号**：调用 `baoyu-markdown-to-html` 技能，将 markdown 转换为富文本 HTML，支持代码高亮、数学公式、图表等高级排版
   - **微博长文**：将 markdown 转换为纯文本格式
     - 去掉 markdown 标记符号（`**`、`#`、`` ` ``、`>`等）
     - 保留文本内容和段落结构
     - 使用 emoji 或特殊符号（如 ▪️、→）替代列表标记
     - 支持长文本，无字数限制
   - **小红书长文**：将 markdown 转换为小红书风格纯文本
     - 去掉 markdown 标记符号
     - 使用 emoji 增强表现力（💡、✨、📌等）
     - 段落间用空行分隔，提升可读性
     - 关键词标签放在文末（#标签 格式）
